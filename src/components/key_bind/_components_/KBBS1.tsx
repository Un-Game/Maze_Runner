...
const KBBS1 = () => {
  ...
  const handleKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^[a-zA-Z]$/.test(value)) {
      updateKeybind("skillOne", value.toUpperCase());
    } else if (value === "") {
      updateKeybind("skillOne", "");
    }
  };

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === keybinds.skillOne) {
      ...
      <div className="w-full flex gap-20 justify-between">
        ...
        <div className="flex justify-end items-end pr-[100px]">
          <input
            ...
            type="text"
            maxLength={1}
            value={keybinds.skillOne}
            onChange={handleKeyChange}
            onFocus={() => {
              if (keybinds.skillOne !== "") updateKeybind("skillOne", "");
            }}
            onClick={() => {
              if (keybinds.skillOne !== "") updateKeybind("skillOne", "");
            }}
