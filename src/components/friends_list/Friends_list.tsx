...
export default function Friend_list(props: props) {
  ...
  const fetchIncoming = async () => {
    try {
      const res = await axios.get(
        `https://maze-runner-backend-1.onrender.com/request/incoming/${user._id}`
      );
  ...
  const fetchUser = async (id) => {
    ...
    try {
      const response = await axios.get(
        `https://maze-runner-backend-1.onrender.com/user/${id}`
      );
  ...
  const deleteFriend = async (id) => {
    ...
    try {
      await axios.put(
        "https://maze-runner-backend-1.onrender.com/user/removefriend",
        {
          senderId: user._id,
          receiverId: id,
        }
      ...
      await refetchUser();
      socket.emit("friend:request", { sender: id });
  ...
  const searchUser = async () => {
    ...
    try {
      const response = await axios.get(
        `https://maze-runner-backend-1.onrender.com/user/find/${searchInput}`
      );
  ...
  const addFriendButton = async () => {
    setSending("sending");
    try {
      await axios.post("https://maze-runner-backend-1.onrender.com/request/", {
        senderId: user._id,
  ...
  const acceptRequest = async (id) => {
    try {
      const resp = await axios.delete(
        "https://maze-runner-backend-1.onrender.com/request/accept",
        {
          data: {
            senderId: id,
            receiverId: user._id,
  ...
  const declineRequest = async (id) => {
    try {
      const resp = await axios.delete(
        "https://maze-runner-backend-1.onrender.com/request/decline",
        {
          data: {
            senderId: id,
            receiverId: user._id,
        ...
        <div className="w-full h-full flex flex-col gap-[20px]">
          <div className="text-[25px] pb-[20px] border-b border-cyan-400 flex justify-between">
            ...
            <Dialog>
              <DialogTrigger>
                <UserPlus
                  className="border  bg-cyan-700 rounded-[5px] p-[4px]"
                  size={32}
                />
              </DialogTrigger>
              <DialogContent className="bg-black">
                <DialogTitle className="text-center text-cyan-300 text-[25px]">
                  Add friend
                </DialogTitle>
                <div className="flex flex-col gap-[10px]">
                  ...
                  <div className="flex w-full gap-[20px]">
                    <input
                      type="text"
                      className="w-full border outline-none text-[18px] rounded-[5px] py-[5px] px-[10px]"
                      onChange={(e) => {
                    ...
                    />
                    <button
                      className="text-[20px] hover:text-cyan-300"
                      onClick={searchUser}
                    >
                      Find
                    </button>
                  ...
                  {searchResult === "loading" ? (
                    ...
                    <div className="w-full flex justify-between mt-[20px] text-[20px]">
                      ...
                      <button
                        className="text-[15px] border py-[5px] bg-cyan-400/60 rounded-[5px] w-[100px]"
                    ...
                    searchResult === "" && (
                      <div className="w-full ml-[5px] text-red-400/80">
                        Player not found
                      </div>
                    )
                  )}
                    ...
                    >
                      <div className="flex gap-[15px] w-full cursor-pointer">
                        <div className="min-w-[40px] h-[40px] rounded-full justify-center items-center">
                          <img
                            src={friend.avatar ? friend.avatar : "./globe.svg"}
                            className="w-[40px] h-[40px] object-cover rounded-full"
                          />
                        </div>
                        <div className="text-[20px] w-full truncate">
                          {friend.username}
                        </div>
                      </div>
                    </PopoverTrigger>
                    ...
                    >
                      {error ? (
                        ...
                        <div className="flex flex-col gap-[15px]">
                          ...
                          <div className="">
                            Level: {Math.floor(viewFriend.exp / 100) + 1} <br />{" "}
                            Current exp: {viewFriend.exp % 100}
                          </div>
                          <div className="flex justify-end">
                            {deletionLoading ? (
                              ...
                              <button onClick={() => setDeletionConfirm(true)}>
                                <UserRoundX className="text-red-400" />
                              </button>
                            )}
                          </div>
                        </div>
          ...
          <div className="w-full border-b">
            <Collapsible>
              <CollapsibleTrigger className="w-full pb-[10px] flex justify-center">
                <div className="text-20px">Incoming requests ~ </div>&nbsp;
                <div className="bg-gray-600 px-[5px] rounded-[8px]">
              ...
              <CollapsibleContent>
                {incomingRequests.map((req, index) => {
                  return (
                    <div
                      key={index}
                      className="w-full flex justify-between p-[10px]"
                    >
                      ...
                      <div className="flex gap-[20px]">
                        <button
                          className="w-[25px] h-[25px] bg-cyan-400 rounded-[5px]"
                          onClick={() => acceptRequest(req.senderId)}
                        >
                          <Check />
                        </button>
                        <button
                          className="w-[25px] h-[25px] bg-red-400 rounded-[5px]"
